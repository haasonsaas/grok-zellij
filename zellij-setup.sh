...
# @type string LAYOUT_PATH - Path to the layout configuration file
LAYOUT_PATH="$(dirname "$0")/layouts/default.yaml"

# Parse command line arguments
# @type string SESSION_NAME - Name of the Zellij session to create/attach to
SESSION_NAME="grok-project"
# @type int DRY_RUN - Flag indicating dry-run mode (0=false, 1=true)
DRY_RUN=0
# @type string ATTACH_SESSION - Name of existing session to attach to (empty if not attaching)
ATTACH_SESSION=""
# @type string CONFIG_FILE - Path to Zellij configuration file (empty if not specified)
CONFIG_FILE=""
# @type string MAX_PANES - Maximum number of panes allowed (empty if not specified)
MAX_PANES=""
# @type int NEW_SESSION - Flag to force creation of new session (0=false, 1=true)
NEW_SESSION=0
# @type string THEME - Zellij theme name (empty if not specified)
THEME=""
# @type int VERBOSE - Flag for verbose output (0=false, 1=true)
VERBOSE=0
# @type string $1 - Current command line argument being processed
while [[ $# -gt 0 ]]; do
	case $1 in
	--help)
		show_help
...
if [[ "${NEW_SESSION}" = 1 ]]; then
	# @type string CMD - The complete zellij command to be executed
	CMD="zellij"
	if [[ -n "${CONFIG_FILE}" ]]; then
		CMD="${CMD} --config \"${CONFIG_FILE}\""
	fi
